...
export function timestampMatcher(node: ESTree.Node) {
  ...
  // vdl-patch: explicit `return` at the visitor-callback end so our
  // tsconfig's `noImplicitReturns` is happy. Upstream returns undefined
  // implicitly — same "continue walking" semantics.
    ...
    if (innerNode.type === 'ObjectExpression') {
      for (const prop of innerNode.properties) {
        if (
          ...
          prop.key.name === 'signatureTimestamp'
        ) {
          foundObject = prop;
          return WALK_STOP;
        }
