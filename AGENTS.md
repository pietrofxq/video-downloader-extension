...
18. **YouTube WEB-client responses are SABR — adaptive URLs are gone.** Since some point in 2024–25, `ytInitialPlayerResponse.streamingData.adaptiveFormats[]` ships full metadata (itag, codec, bitrate, contentLength, ...) but no `url`, no `signatureCipher`, no `cipher`. Only the progressive itag=18 fallback still ships a URL. The YouTube adapter (`extension/adapters/youtube.ts`) handles this with a fallback ladder: scrape inline first (cheap, still useful for `videoDetails`); if no adaptive variants surface, POST to `/youtubei/v1/player` with a non-WEB InnerTube client (`extension/adapters/youtube-clients.ts`). The fetch **must originate from the content-script context** because the InnerTube endpoint validates the request's Origin / Referer — same constraint as #15. SW-originated InnerTube fetches will be rejected.

    **v0.12 correction — read this before touching the client list.** An earlier version of this note claimed the mobile clients were unusable because Chrome forbids setting `User-Agent` from a content script and IOS / ANDROID answered `400`. **That diagnosis was wrong**, and it was expensive: it sent the investigation toward a BotGuard / poToken project that turned out to be unnecessary. The `400` comes from sending the web session's `Authorization: SAPISIDHASH` header to a client that rejects that auth shape — not from the User-Agent. Drop that header and supply `visitorData` and `ANDROID_VR` answers `OK` under the ordinary Chrome UA, returning adaptive URLs the CDN actually serves (verified to 2160p, and its URLs carry no `n` param so the signature solver is skipped entirely). Two rules follow: a bare InnerTube `400` means "malformed for this client", not "wrong UA" — vary the auth shape before writing a client off; and **URLs from different clients are not interchangeable**, because the CDN gates on the acquisition path (`WEB_CREATOR` / `MWEB` URLs 403 without a poToken while `ANDROID_VR` URLs serve fine).
19. **Adapter `discoverStreams` is async-capable.** The Adapter contract returns `DiscoveredStream[] | Promise<DiscoveredStream[]>`. Synchronous adapters (`default`, `hotmart`) stay zero-cost; YouTube's fallback ladder is the async case. The page-content call site wraps with `Promise.resolve()` so both shapes work without ceremony.
...
6. **`mvhd` / `tkhd` / `mdhd` duration fields must be patched — and the total must live in `mehd`, not the headers.** mux.js leaves the header durations at `0xFFFFFFFF` (the fMP4 "unknown duration, derive from samples" sentinel). MSE-based players honor the sentinel; VLC reads it literally — `0xFFFFFFFF / 90000 ≈ 47 721 s ≈ 13.26 hours`. The first fix wrote the real per-track total into `mvhd`/`tkhd`/`mdhd`. That played correctly in VLC/Chrome/ffmpeg but **doubled the duration in QuickTime/AVFoundation only**: QuickTime computes a fragmented file's duration as `mvhd.duration + Σ(fragment durations)`, so a populated base-movie duration + fragments covering the same span = 2×. The correct layout (what FFmpeg's `empty_moov` output produces, reads right *everywhere* including QuickTime): **zero** the `mvhd`/`tkhd`/`mdhd` durations — the base movie carries no samples — and declare the real total (`longest-track-seconds × movie_timescale`) in an **`mehd`** (movie-extends-header) box. mux.js emits no `mehd`, so we inject a 16-byte v0 one as the first child of `mvex` (`injectMehdPlaceholder`) **while the init segment is still ahead of the fragments** — growing it later would shift every moof/mdat — then `patchHeaderDurations` fills its `fragment_duration` (body `+4`). Box duration-field offsets still differ per box: `tkhd` `+20`/`+28` (v0/v1), `mvhd`/`mdhd` `+16`/`+24`, `mehd` `+4`/`+4` — keep the helper functions separate. Adding `mehd` is safe without touching downstream offsets only because the fragments use moof-relative addressing (default-base-is-moof) and the moov sample tables are empty.
...
8. **The whole patching pipeline lives in `extension/offscreen/remux.ts`** and has dedicated regression tests (`extension/offscreen/remux.test.ts`): one asserts the `ftyp / moov / moof / mdat / moof / mdat` shape with two trafs per moof; another asserts the QuickTime fix — `mvhd`/`tkhd`/`mdhd` durations zeroed and a non-zero `mehd.fragment_duration` in `mvex`. If you change anything in that file, run the tests — they're the firewall against re-introducing any of the above bugs.
