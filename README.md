...
> - **YouTube** — **progressive 360p (itag=18) only on current YouTube.** YouTube migrated WEB-client responses to SABR (Server-Adaptive Bit Rate), which ships format metadata without per-format URLs. The full adaptive HD pipeline (signatureCipher + n-param decipher, two-stream fMP4 combine muxer, parallel video+audio fetch) is wired up and lights up the moment URLs come back — that's v0.11.2's job, which switches the catalog scrape to a non-WEB InnerTube client (IOS / TVHTML5_SIMPLY_EMBEDDED_PLAYER). Public non-DRM watch pages only; rentals, age-gated, region-locked, and live content are out of scope. VP9 / AV1 variants will surface in the picker once URLs return but won't download (the muxer is stream-copy in mp4; lifting that needs re-encoding, which the project rules out).
