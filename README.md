...
> - **YouTube** — adaptive HD (1080p) and 4K (2160p) for **AVC** and **AV1** codecs. The full pipeline is wired: InnerTube scrape for the catalog (ANDROID_VR first — it is the client whose media URLs the CDN actually serves; WEB_CREATOR / MWEB / TVHTML5 follow as inventory fallbacks), visitorData session binding, signatureCipher + n-param decipher where the URLs need it, OPFS-staged chunked Range fetches, and an OPFS-streaming two-stream combiner that de-fragments video + audio into one plain MP4 (so seeking + A/V sync are correct in VLC). Long 4K downloads are supported — a 2h44m 2160p video is ~8 GB and needs roughly twice that in browser storage while it stages, which is why the extension requests `unlimitedStorage`. Public non-DRM watch pages only; rentals, age-gated, region-locked, and live content are out of scope. **VP9** variants surface in the picker but stay rejected — they're in fragmented WebM, which needs a different container muxer (deferred, picked up as needed). Multi-dub videos get an audio-track picker; default pairs the original track.
